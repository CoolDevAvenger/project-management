...
@push('styles')
    ...
    <style>
        ...
        .highlighted-day {
            background-color: yellow!important; /* Change this to the desired background color */
            color: black!important; /* Change this to the desired text color */
...
{{--        // Find all the elements with the class "fc-daygrid-day"--}}
{{--        var dayElements = document.querySelectorAll('.fc-daygrid-day');--}}

{{--        // Loop through the elements and compare the date attribute to the end_date--}}
{{--        dayElements.forEach(function(dayElement) {--}}
{{--            var dateAttribute = dayElement.getAttribute('data-date');--}}

{{--            if (dateAttribute === end_date) {--}}
{{--                // Apply styling to the last date--}}
...
{{--    </script>--}}
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Your code to run after the document is fully loaded
            var start_date = '{{$start_date}}';
            var end_date = '{{$end_date}}';
            var formattedStartDate = moment(start_date, 'DD-MM-YYYY').format('YYYY-MM-DD');
            var formattedEndDate = moment(end_date, 'DD-MM-YYYY').format('YYYY-MM-DD');

            var dayElements = document.querySelectorAll('.fc-daygrid-day');
            dayElements.forEach(function(dayElement) {
                var dateAttribute = dayElement.getAttribute('data-date');
                console.log(dateAttribute === formattedStartDate);
                if (dateAttribute >= formattedStartDate && dateAttribute <= formattedEndDate) {
                    dayElement.classList.add('highlighted-day');
                }
            });

            // ... your existing FullCalendar setup code ...
    ...
    <script>
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventDidMount: function(info) {
                $(info.el).find('.fc-daygrid-day-events').append(
                            `<button class="btn btn-secondary f-14">info.event.title</button>`);
                $(info.el).popover({
                    title:info.event.extendedProps.shortTitle+"<small class='d-block'>"+info.event.extendedProps.assingedTo+"</small>",
                    content:info.event.extendedProps.project,
                    html:true,
                    placement:'top',
                    container: "body",
        ...
        // element.find('.fc-title').after("<span>"+event.title+"</span>")
