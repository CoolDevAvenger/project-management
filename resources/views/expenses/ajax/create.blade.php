            ...
            <div class="add-client bg-white rounded">
                ...
                <div class="row p-20">
                            ...
                            <x-forms.input-group>
                                <select class="form-control select-picker" name="user_id" id="user_id"
                                    ...
                                    @foreach ($employees as $item)
                                        <x-user-option :selected="$item->id == user()->id" :user="$item" />
                                    @endforeach
                                </select>
                            </x-forms.input-group>
                        </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <x-forms.input-group>
                            <select class="form-control select-picker" required name="category_id" id="expense_category_id"
        ...
        $('#save-expense-form').click(function() {
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    if(page == 'estimate'){
                            console.log("expense dropdown", response);
                            if(item_no){
                                $('#add_expense' + item_no).html(response.options);
                                $('#add_expense' + item_no).trigger('change');
                                $('#add_expense' + item_no).selectpicker('refresh');
                                $('#expense_category_id' + item_no).html(response.categoryOptions);
                                $('#expense_category_id' + item_no).selectpicker('refresh');
                            }else {
                                $('#add_expense').html(response.options);
                                $('#add_expense').trigger('change');
                                $('#add_expense').selectpicker('refresh');
                                $('#expense_category_id').html(response.categoryOptions);
