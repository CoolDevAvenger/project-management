                    ...
                    <div class="select-others">
                        <select class="form-control select-picker" name="project_id" id="project_id2" data-container="body" data-live-search="true" data-size="8">
                            ...
                            @foreach ($projects as $project)
                                <option data-content="{{ $project->project_name }}" value="{{ $project->id }}"  @if (isset($taskProjectId) && $taskProjectId == $project->id) selected @endif>{{ mb_ucwords($project->project_name) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
    ...
    <div class="content-wrapper">
        ...
        <div class="d-flex justify-content-between action-bar">
            ...
            @if (isset($taskProjectId))
                <div class="col-md-5">
                    <h5 class="expense-project-name font-weight-bold"></h5>
                </div>
                <div class="col-md-2 text-right">
                    <a href="{{route('projects.index')}}" class="btn btn-outline-secondary ml-1">@lang('app.back')</a>
                </div>
            @endif
            <x-datatable.actions>
...
@push('scripts')
    ...
    <script>
        $('#expenses-table').on('preXhr.dt', function(e, settings, data) {
            ...
            var projectName = $('#project_id2 option:selected').data("content");
            $(".expense-project-name").html(projectName);
