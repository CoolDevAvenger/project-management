                ...
                <div class="row p-20">
                    <div class="col-lg-12">
                        <div class="row">
                            ...
                            <div class="col-lg-4 col-md-6">
                                <x-forms.number class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.cost')"
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <x-forms.input-group>
                                    <select class="form-control select-picker" name="category_id" required
                                            id="service_category_id" data-live-search="true">
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <x-forms.input-group>
                                    <select class="form-control select-picker" name="sub_category_id" required id="sub_category_id" data-live-search="true">
                                ...
                                <x-forms.input-group>
                                    <select class="form-control select-picker" name="tax[]" id="tax_id"
                                        ...
                                        @foreach ($taxes as $tax)
                                            <option @if ($tax->is_default == 1) selected @endif value="{{ $tax->id }}">{{ strtoupper($tax->tax_name) }}:
                                                {{ $tax->rate_percent }}%
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <x-forms.input-group>
                                    <select class="form-control" name="unit_type" id="unit_type_id"
                            ...
                            <div class="col-lg-12 @if (isset($page) && $page == "estimate") d-none @endif">
...
<script>
    $(document).ready(function () {
        ...
        $('#service_category_id').change(function (e) {
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    if (productDropzone.getQueuedFiles().length > 0) {
                        ...
                        if(page == 'estimate'){
                            console.log("product dropdown", response);
                            if(item_no){
                                $('#add-products' + item_no).html(response.options);
                                $('#add-products' + item_no).trigger('change');
                                $('#add-products' + item_no).selectpicker('refresh');
                                $('#product_category_id' + item_no).html(response.categoryOptions);
                                $('#product_category_id' + item_no).selectpicker('refresh');
                                $('#product_sub_category_id' + item_no).html(response.subCategoryOptions);
                                $('#product_sub_category_id' + item_no).selectpicker('refresh');
                            }else {
                                $('#add-products').html(response.options);
                                $('#add-products').trigger('change');
                                $('#add-products').selectpicker('refresh');
                                $('#product_category_id').html(response.categoryOptions);
                                $('#product_category_id').selectpicker('refresh');
                                $('#product_sub_category_id').html(response.subCategoryOptions);
