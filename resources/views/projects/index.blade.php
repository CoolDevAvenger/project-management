...
if(isset($_GET['statusId'])){
    ...
    $dStatus = App\Models\ProjectStatusSetting::where('id', $statusId)->pluck('status_name')->first();
}
@endphp
            ...
            <div class="select-status">
                <select class="form-control select-picker" name="status" id="status" data-live-search="true" data-size="8">
                    ...
                    @foreach ($projectStatus as $status)
                        <option @if (isset($dStatus) && $dStatus == $status->status_name) selected @endif value="{{$status->status_name}}">{{ ucfirst($status->status_name) }}</option>
                    @endforeach
                </select>
            </div>
        </div>
...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        @if(request()->path() == 'account/tasks-pro')
        <a href="{{route('tasks.index')}}" class="btn-primary rounded f-14 p-2 mr-3 float-left">
                ...
                <x-datatable.actions>
                    ...
                    <div class="select-status mr-3 d-none quick-action-field" id="change-status-action">
                        <select name="status" class="form-control select-picker">
                            <option value="no task">@lang('app.noTask')</option>
