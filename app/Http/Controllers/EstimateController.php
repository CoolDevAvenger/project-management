...
class EstimateController extends AccountBaseController
{
    ...
    public function create()
    {
        ...
        $this->taxes = Tax::where('is_default', 1)->get();
        $this->products = Product::all();
        $this->serviceCategories = ProductCategory::all();
        $this->serviceSubCategories = ProductSubCategory::all();
        $this->projectCategories = ProjectCategory::all();
        $this->projectSubCategories = ProjectSubCategory::all();
        $this->expenses = Expense::where('project_id', null)->where('estimate_id', null)->get();
        $this->expenseCategories = ExpensesCategory::all();
    ...
    public function store(StoreEstimate $request)
    {
        ...
        foreach ($items as $key => $itm) {
            ...
            if (is_null($request->product_category_id[$key])) {
                return Reply::error(__('messages.productCategoryBlank'));
            }
            if (is_null($product_sub_category_id[$key])) {
                return Reply::error(__('messages.productSubCategoryBlank'));
            }
            if (!is_null($request->{"expense_category_id" . $key})) {
                if (is_null($request->{"expense_item_id". $key})) {
                    return Reply::error(__('messages.ExpenseBlank'));
                }
            }
            // if (!is_null($expense_item_id[$key])) {
            //     $hasDuplicates = count($expense_item_id) > count(array_unique($expense_item_id));
            //     if ($hasDuplicates) {
            //         return Reply::error(__('messages.duplicateExpense'));
            //     }
            // }
        }
        $estimate = new Estimate();
        $estimate->client_id = $request->client_id;
        $estimate->project_category_id = $request->project_category_id;
        $estimate->project_sub_category_id = $request->project_sub_category_id;
        ...
        $estimate->total_margin = $request->total_margin_amount;
        $estimate->total_margin_percent = $request->total_margin_percent;
        $estimate->total_hours = $request->total_hours;
        $estimate->total_service = $request->total_service;
        $estimate->total_expense = $request->total_expense;
        $estimate->total_cost = $request->total_cost;
        ...
        $estimate-> start_date= Carbon::createFromFormat($this->company->date_format, $request->start_date)->format('Y-m-d');
        $estimate->end_date = Carbon::createFromFormat($this->company->date_format, $request->end_date)->format('Y-m-d');
        $estimate->payment_method = $request->payment_method;
        $estimate->payment_type = $request->payment_type;
        $estimate->total_tax_amount = $request->total_tax_amount;
        ...
        foreach ($estimate->items as $item){
            if(!is_null($item->expense_item_id) && $item->expense_item_id > 0){
                $expense_ids = json_decode($item->expense_item_id);
                if (isset($expense_ids) && count($expense_ids) > 0){
                    foreach ($expense_ids as $data){
                        $expense = Expense::findOrFail($data);
                        if(!is_null($expense)) {
                            $expense->estimate_id = $item->estimate_id;
                            $expense->estimate_item_id = $item->id;
    ...
    public function edit($id)
    {
        ...
        if($this->estimate->status == 'accepted'){
            ...
            $this->expenses = Expense::where('estimate_id', $id)
    ...
    public function update(StoreEstimate $request, $id)
    {
        ...
        $tax = $request->taxes;
        $expnese_tax = $request->expnese_tax;
        ...
        foreach ($items as $key => $itm) {
            ...
            if (is_null($product_category_id[$key])) {
                return Reply::error(__('messages.productCategoryBlank'));
            }
            if (is_null($product_sub_category_id[$key])) {
                return Reply::error(__('messages.productSubCategoryBlank'));
...
//            if (!empty($item_ids)) {
//
//                if(EstimateItem::whereNotIn('id', $item_ids)->where('estimate_id', $estimate->id)->delete()){
//                    $expense = Expense::where('estimate_item_id', $estimateItem->id)->first();
//                    if (!is_null($expense)){
//                        $expense->delete();
//                    }
//                    // if(!is_null($expense)) {
//                    //     $expense->estimate_id = null;
//                    //     $expense->estimate_item_id = null;
//                    //     $expense->last_updated_by = user()->id;
//                    //     $expense->save();
//                    // }
//                }
//            }
            ...
            foreach ($items as $key => $item) {
                ...
                try {
                    $estimateItem = EstimateItem::findOrFail($item_ids[$key]);
                }
                catch(Exception) {
                ...
                $estimateItem->item_id = $product_id[$key];
                $estimateItem->item_name = $item;
               $estimateItem->item_summary = $itemsSummary[$key];

                $estimateItem->product_category_id = $product_category_id[$key];
                $estimateItem->product_category_name = $product_category_name[$key];
                $estimateItem->product_sub_category_id = $product_sub_category_id[$key];
                $estimateItem->product_sub_category_name = $product_sub_category_name[$key];
                $estimateItem->service_margin_percent = $service_margin_percent[$key];
                ...
                $estimateItem->unit_id = $product_unit_id[$key];
                $estimateItem->unit_name = $product_unit_name[$key];
                $estimateItem->item_price = round($cost_per_item[$key], 2);
                $estimateItem->unit_price = round($cost_per_item[$key], 2);
                $estimateItem->amount = round($amount[$key], 2);

                $estimateItem->cost = $cost[$key];
                $estimateItem->margin = $margin[$key];
                $estimateItem->expense_category_id = json_encode($expense_category_id);
                $estimateItem->expense_category_name = ExpensesCategory::where('id', $expense_category_id[$key])->pluck('category_name')->first();
                $estimateItem->expense_item_id = json_encode($expense_item_id);
                $estimateItem->expense_item_name = json_encode($expense_item_name);
                $estimateItem->expense_price = json_encode($expense_price);
                $estimateItem->expense_margin_percent = json_encode($expense_margin_percent);
                $estimateItem->expense_amount = json_encode($expense_amount);
                $estimateItem->expense_cost = json_encode($expense_cost);
                $estimateItem->expense_margin = json_encode($expense_margin);
                $estimateItem->expnese_tax = json_encode($expnese_tax);
                $estimateItem->expense_tax_amount = json_encode($expense_tax_amount);

                $estimateItem->taxes = ($tax ? (array_key_exists($key, $tax) ? json_encode($tax[$key]) : null) : null);
                $estimateItem->save();
//                if (isset($expense_item_id) && count($expense_item_id) > 0){
//
//                        $preExpense = Expense::where('id', $expense_item_id[$key])->first();
//                        if(!is_null($preExpense)) {
//                            if(is_null($expense_item_name[$key])){
//                                // $expense->estimate_id = null;
//                                // $expense->estimate_item_id = null;
//                                Expense::where('id', $expense_item_id[$key])->delete();
//                            }
//                            if($preExpense->id != $expense_item_id[$key]){
//                                // $expense->estimate_id = null;
//                                // $expense->estimate_item_id = null;
//                                Expense::where('id', $expense_item_id[$key])->delete();
//                            }
...
//                            $estExpense = Expense::where('id',$expense_item_id[$key])->first();
...
//                }
        ...
        foreach ($estimate->items as $key=>$item){
            if(!is_null($expense_item_id[$key]) && $expense_item_id[$key] > 0){
                $expense = Expense::findOrFail($expense_item_id[$key]);
                if(!is_null($expense)) {
                    $expense->estimate_id = $item->estimate_id;
                    $expense->estimate_item_id = $item->id;
                    $expense->status = 'pending';
        ...
        if($request->type == "accept"){
            $project = Project::where('estimate_id', $estimate->id)->first();
            if(is_null($project)) {
                $estimate = Estimate::findOrFail($id);
                $accept = new AcceptEstimate();
                $accept->full_name = $estimate->client->name;
                $accept->estimate_id = $estimate->id;
                $accept->email = $estimate->client->email;
                $imageName = null;
                $accept->save();
                ...
                $project = new Project();
                $project->project_name = "Project " . $estimate->projectCategory->category_name;
                $project->project_short_code = "PR-00". rand(100,999) . Project::count();
                $project->company_id = $estimate->company_id;
                $project->estimate_id = $estimate->id;
                $project->project_budget = $estimate->total;
                $project->start_date = $estimate->start_date;

                $project->total_estimate_hour = $estimate->total_hours;
                $project->total_estimate_service = $estimate->total_service;
                $project->total_estimate_cost = $estimate->total_cost;
                $project->total_estimate_margin = $estimate->total_margin;
                $project->total_estimate_expense = $estimate->total_expense;
                ...
                $project->currency_id = $estimate->currency_id;
                $project->category_id = $estimate->project_category_id;
                $project->sub_category_id = $estimate->project_sub_category_id;
                ...
                foreach ($estimate->items as $item) :
                    ...
                    [
                        ...
                        'estimate_margin_percent' => $item->service_margin_percent,
                        'estimate_hrs' => $item->unit_name == "Price\hrs" ? $item->quantity : 0,
                        'estimate_price' => $item->unit_price,
                        'estimate_amount' => $item->amount,
                        'estimate_cost' => $item->cost,
                        'estimate_margin' => $item->margin,
                        'estimate_expense_price' => $item->expense_price,
                        'estimate_expense_amount' => $item->expense_amount,
                        'estimate_expense_cost' => $item->expense_cost,
                        'estimate_expense_margin' => $item->expense_margin,
                        'unit_id' => $item->unit_id,
                        'unit_name' => $item->unit_name,
                        'task_short_code' => $project->project_short_code . '-' . Task::count(),
        ...
        // return Reply::success(__('messages.updateSuccess'), ['redirectUrl' => $redirectUrl]);
        // return Reply::success(__('messages.updateSuccess'), route('estimates.index'));
        return Reply::redirect(route('estimates.index'), __('messages.updateSuccess'));
        // return redirect(route('estimates.index'))->with('message', __('messages.updateSuccess'));
    ...
    public function destroy($id)
    {
        ...
        if($estimate->project_id==""){
            foreach ($estimate->items as $item){
                if(!empty($item->expense_item_id) && $item->expense_item_id > 0){
                    Expense::destroy($item->expense_item_id);
                }
            }
        }

        if($estimate->project_id != ""){
            return Reply::error(__('messages.cannotDelete'));
        }
