...
class TasksDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) {
            ...
            if ($this->editTaskPermission == 'all'
                ...
                if($projectId && $projectId != 'all'){
                    $action .= '<a class="dropdown-item" href="' . route('tasks.edit', [$row->id]) . '?projectId=' .$projectId . '">
        ...
        $datatables->editColumn('users', function ($row) {
            ...
            foreach ($row->users as $key => $member) {
                if ($key < 4) {
                    ...
                    $members .=  '<div>Assigned</div>';
                    //$members .= '<div class="taskEmployeeImg rounded-circle ' . $position . '" style="left:  ' . ($key * 13) . 'px"><a href="' . route('employees.show', $member->id) . '">Assigned</a></div> ';
                }
            }
        ...
        $datatables->addColumn('estimate_hrs', function ($row) {
            return $row->unit_name =="Fixed Price"  || $row->unit_name =="Hrs" ? "--" : $row->estimate_hrs;
        });
        $datatables->addColumn('unit_name', function ($row) {
            return $row->unit_name ;
        });
        $datatables->addColumn('estimate_price', function ($row) {
            return ucfirst('€'.$row->estimate_price);
        });
        $datatables->addColumn('actual_cost', function ($row) {
            // return $row->actual_cost > 0 ? '€'.$row->actual_cost : "--";
            $actual_cost = '--';

            if ($row->timeLogged) {
                $actual_cost = $row->timeLogged->sum('earnings');
            }
            return currency_format($actual_cost, $row->currencyId);
        });
    ...
    public function query(Task $model)
    {
        ...
        $model->leftJoin('users as creator_user', 'creator_user.id', '=', 'tasks.created_by')
            ...
            ->selectRaw('tasks.id, tasks.task_short_code, tasks.estimate_price, tasks.actual_cost, tasks.estimate_hrs, tasks.unit_name, tasks.added_by, projects.project_name, projects.project_admin, tasks.heading, client.name as clientName, creator_user.name as created_by, creator_user.image as created_image, tasks.board_column_id,
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('app.project') => ['data' => 'project_name', 'visible' => is_null($projectId) ? true : false, 'name' => 'projects.project_name', 'title' => __('app.project')],
            __('app.estimateHour') . ' ' => ['data' => 'estimate_hrs', 'name' => 'estimate_hrs', 'title' => __('app.estimateHour')],
            __('app.unitName') . ' ' => ['data' => 'unit_name', 'name' => 'unit_name', 'title' => __('app.unitName')],
            __('app.estimatePrice') . ' ' => ['data' => 'estimate_price', 'name' => 'estimate_price', 'title' => __('app.estimatePrice')],
            __('app.actualCost') . ' ' => ['data' => 'actual_cost', 'name' => 'actual_cost', 'title' => __('app.actualCost')],
