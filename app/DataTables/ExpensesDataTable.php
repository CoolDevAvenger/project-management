...
class ExpensesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
            ...
            if (is_null($row->expenses_recurring_id)) {
                if ($this->editExpensePermission == 'all' || ($this->editExpensePermission == 'added' && user()->id == $row->added_by)) {
                    ...
                    if($projectId && $projectId != 'all'){
                        $action .= '<a class="dropdown-item openRightModal" href="' . route('expenses.edit', [$row->id]) . '?projectId=' .$projectId . '">
        ...
        $datatables->editColumn('status', function ($row) {
            if (
                ...
                $actualCostAssigned ="";
                if ($row->actual_price != '' && $row->actual_price != 0) {
                    $actualCostAssigned = 'Actual cost added';
                }
                ...
                $status .= ' value="pending" data-content="<i class=\'fa fa-circle mr-2 text-yellow\'></i> ' . __('app.pending') .'<span class=\'ml-2 badge badge-secondary pr-1\'>' . $actualCostAssigned . '</span>">' . __('app.pending') . '</option>';
                $status .= '<option ';
        ...
        $datatables->addColumn('actual_price', function ($row) {
            return !is_null($row->actual_price) && $row->actual_price > 0 ? $row->currency_symbol.$row->actual_price : '--';
        });
        $datatables->addColumn('project_name', function ($row) {
            return !is_null($row->project_id) ? (!is_null($row->project->project_name) ? $row->project->project_name : '--') : '--';
        });
        $datatables->addColumn('estimate_name', function ($row) {
            return !is_null($row->estimate_id) ? (!is_null($row->estimate_id) ? 'EST#00'.$row->estimate_id : '--') : '--';
        });

        $datatables->addColumn('task_name', function ($row) {
            return !is_null($row->heading) ? $row->heading : '--';
        });

        $datatables->addColumn('expense_category', function ($row) {
    ...
    public function query()
    {
        ...
        $model = Expense::with('currency', 'user', 'user.employeeDetail', 'user.employeeDetail.designation', 'user.session')
            ->select('expenses.id', 'expenses.item_name', 'expenses.user_id', 'expenses.category_id','expenses.price', 'expenses.actual_price', 'tasks.heading', 'users.name', 'expenses.purchase_date', 'expenses.currency_id', 'expenses.estimate_id','expenses.project_id','currencies.currency_symbol', 'expenses.status', 'expenses.purchase_from', 'expenses.expenses_recurring_id', 'designations.name as designation_name', 'expenses.added_by')
            ->join('users', 'users.id', 'expenses.user_id')
            ->leftJoin('tasks', 'tasks.estimate_item_id', '=', 'expenses.estimate_item_id')
        ...
        if ($request->projectId == 'all' || $request->projectId == '') {
            $model->where('expenses.estimate_id', null);
            $model->where('expenses.project_id', null);
        }
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('modules.expenses.expenseCategory') => ['data' => 'expense_category', 'name' => 'expense_category', 'exportable' => false, 'title' => __('modules.expenses.expenseCategory')],
            __('app.task') => ['data' => 'task_name', 'name' => 'task_name', 'visible' => true, 'title' => __('app.task')],
            __('app.menu.itemName') => ['data' => 'export_item_name', 'name' => 'export_item_name', 'visible' => false, 'title' => __('modules.expenses.itemName')],
            __('app.estimate') => ['data' => 'estimate_name', 'name' => 'estimate_name', 'visible' => true, 'title' => __('app.estimate')],
            __('app.project') => ['data' => 'project_name', 'name' => 'project_name', 'visible' => is_null($projectId) ? true : false, 'title' => __('app.project')],
            __('app.estimatePrice') => ['data' => 'price', 'name' => 'price', 'title' => __('app.estimatePrice')],
            __('app.actualPrice') => ['data' => 'actual_price', 'name' => 'actual_price', 'title' => __('app.actualPrice')],
            ...
            __('modules.expenses.purchaseFrom') => ['data' => 'purchase_from', 'name' => 'purchase_from', 'visible' => false, 'title' => __('modules.expenses.purchaseFrom')],
            __('modules.expenses.purchaseDate') => ['data' => 'purchase_date', 'visible' => false, 'name' => 'purchase_date', 'title' => __('modules.expenses.purchaseDate')],
