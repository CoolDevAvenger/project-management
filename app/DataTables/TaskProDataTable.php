...
class TaskProDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->editColumn('project_name', function ($row) {
            ...
            return '<div class="media align-items-center">
                    ...
                    <h5 class="mb-0 f-13 text-darkest-grey"><a href="' . route('projects.show', [$row->id]) . '?tab=tasks">' . ucfirst($row->project_name) . '</a></h5>
                    <p class="mb-0">' . $pin . '</p>
                    </div>
                </div>';
        });
        ...
        $datatables->addColumn('client_email', function ($row) {
            ...
            ->addColumn('estimate_amount', function ($row) {
                return currency_format($row->project_budget, $row->currency_id);
            })
        ...
        $datatables->addColumn('goToTasks', function ($row) {
           return '<a href="' . route('tasks.index') . '?projectId='.$row->id.'" class="btn btn-outline-secondary">Tasks</a>
            <a href="' . route('expenses.index') . '?projectId='.$row->id.'" class="btn btn-outline-secondary ml-1">Expenses</a>';
        });
        ...
        $datatables->rawColumns(['project_name', 'action', 'completion_percent', 'members', 'status', 'client_id', 'check','project_short_code', 'goToTasks']);
    ...
    public function query(Project $model)
    {
        ...
        if (!is_null($request->statusId) && $request->statusId != 'all') {
            $model->where('projects.status', DB::raw('(select status_name from `project_status_settings` WHERE id = '. $request->statusId .')'));
        }
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('modules.projects.members') => ['data' => 'members', 'name' => 'members', 'visible' => false, 'exportable' => false, 'width' => '15%', 'title' => __('modules.projects.members')],
            ...
            __('app.project_budget') => ['data' => 'estimate_amount', 'name' => 'estimate_amount', 'title' => __('app.project_budget')],
            ...
            __('app.goToTasks') => ['data' => 'goToTasks', 'name' => 'goToTasks', 'width' => '12%', 'exportable' => false, 'title' => __('app.goToTasks')],
