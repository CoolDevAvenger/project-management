...
class EstimatesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) use ($firstEstimate) {
            ...
            if ($row->status == 'waiting' || $row->status == 'draft' || $row->status == 'accepted') {
                if (
            ...
            if ($row->status == 'waiting' || $row->status == 'draft' || $row->status == 'accepted' || $row->status == 'canceled') {
                if (
